...
for i=2:918
    ...
    if (abs(latest.C1_SchoolClosing(i)-latest.C1_SchoolClosing(i-1))>=1) || (abs(latest.C2_WorkplaceClosing(i)-latest.C2_WorkplaceClosing(i-1))>=1) || (abs(latest.C3_CancelPublicEvents(i)-latest.C3_CancelPublicEvents(i-1))>=1) || (abs(latest.C4_RestrictionsOnGatherings(i)-latest.C4_RestrictionsOnGatherings(i-1))>=1) || (abs(latest.C5_ClosePublicTransport(i)-latest.C5_ClosePublicTransport(i-1))>=1) || (abs(latest.C6_StayAtHomeRequirements(i)-latest.C6_StayAtHomeRequirements(i-1))>=1) || (abs(latest.C7_RestrictionsOnInternalMovement(i)-latest.C7_RestrictionsOnInternalMovement(i-1))>=1) || (abs(latest.C8_InternationalTravelControls(i)-latest.C8_InternationalTravelControls(i-1))>=1)
