...
Trajectories = readtable('Trajectories_07092020_24032022.csv');
Active=Trajectories;
days=height(Trajectories);
Parameters = readtable('EstimatedParameters_7daysWindow.csv');
Beta = readtable('beta_predicted_win7_pred7_mem14_20reps_raw.csv');  %Values of the 4 parameters from System Identification
...
for w=2:21   %21
    ...
    param = Beta{i,w};                            %param=beta*(1-u)            Definition of R0 in SIDARE, proven in paper          
    param1 = Parameters.Param1(i);         %param1=\tilde{beta}*(1-u)   Rate which a vaccinated person infect an unvaccinated person
    param2 = Parameters.Param2(i);         %param2=\hat{beta}*(1-u)     Rate which a vaccinated person infect another vaccinated person
    param3 = Parameters.Param3(i);         %param3=\bar{beta}*(1-u)     Rate which an unvaccinated person infect a vaccinated person
    ...
    Results.PercentageErrorAvg(w,1)=mean(cell2mat(cellPercentageErrorAvgOf7Days));
    Results.PercentageErrorAvg(w,2)=std(cell2mat(cellPercentageErrorAvgOf7Days)); 

    Results.DailyError(w,1)=mean(cell2mat(cellDailyError));
    Results.DailyError(w,2)=std(cell2mat(cellDailyError));

    Results.PercentageErrorSum(w,1)=mean(cell2mat(cellPercentageErrorSumof7Days));
    Results.PercentageErrorSum(w,2)=std(cell2mat(cellPercentageErrorSumof7Days));
